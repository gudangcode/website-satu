	...
	function report_prosentasenilai_count($date1, $date2, $media) {
		...
		if($date1!="" && $date2!=""){
			$sql .= "select count(*) from (
				...
				ifnull(ROUND(((select sum(news_point) from tx_negativenews where media_id = master_media.media_id and news_broadcastdate between '".$date1."' and '".$date2."')/(select sum(news_point) 
			...
			$sql .= "select count(*) from (
				...
				ifnull(ROUND(((select sum(news_point) from tx_negativenews where media_id = master_media.media_id and news_broadcastdate )/(select sum(news_point) from tx_positivenews 
